...
pub fn installed_hotfixes() -> Result<Vec<Hotfix>, WinkitError> {
    ...
    out.sort_by(|a, b| {
        ...
        kb.unwrap_or((i32::MIN, 0, 0))
            .cmp(&ka.unwrap_or((i32::MIN, 0, 0)))
    });
    Ok(out)
}
...
pub fn update_status(max_hotfixes: usize) -> Result<UpdateStatus, WinkitError> {
    ...
    match installed_hotfixes() {
        ...
        Err(e) => {
            status
                .unavailable
                .push(format!("hotfix enumeration unavailable: {}", e.message));
        }
    }
    Ok(status)
}
    ...
    fn newest_first_sorting_prefers_parseable_dates() {
        let mut v = [
            Hotfix {
                hotfix_id: "KB1".into(),
                description: None,
                installed_on: Some("3/1/2026".into()),
            },
            Hotfix {
                hotfix_id: "KB2".into(),
                description: None,
            ...
            Hotfix {
                hotfix_id: "KB3".into(),
                description: None,
                installed_on: Some("9/9/2026".into()),
            },
        ];
        v.sort_by(|a, b| {
            let ka = a.installed_on.as_deref().map(installed_on_sort_key);
