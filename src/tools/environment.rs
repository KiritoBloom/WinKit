...
fn guide_entries() -> Vec<Value> {
    vec![
        ...
        json!({"symptoms": ["overall diagnosis", "full checkup", "something is wrong"], "tool": "system_diagnose", "example_args": {"detail": "compact"}, "notes": "Ranked evidence-backed findings across memory, storage, battery, thermal, Wi-Fi. detail=compact (default) keeps output small; pass detail=detailed for the full per-application table. Start here for vague complaints."}),
