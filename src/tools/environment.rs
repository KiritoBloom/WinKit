...
pub async fn startup_programs_handler(state: Arc<AppState>, _args: Value) -> Result<Value, WinkitError> {
...
pub async fn audit_path_env_handler(state: Arc<AppState>, _args: Value) -> Result<Value, WinkitError> {
...
pub async fn system_update_status_handler(state: Arc<AppState>, args: Value) -> Result<Value, WinkitError> {
...
mod tests {
    ...
    async fn guide_lists_entries_and_every_referenced_tool_exists() {
        ...
        for e in entries {
            ...
            assert!(reg.get(tool).is_some(), "guide references unknown tool '{tool}'");
